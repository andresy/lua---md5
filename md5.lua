...
local S43 = 15
local S44 = 21
local u_char_64 = ffi.typeof("unsigned char[64]")
local u_char_16 = ffi.typeof("unsigned char[16]")
local u_char_8 = ffi.typeof("unsigned char[8]")
local PADDING = u_char_64()
ffi.fill(PADDING, 64, 0)
PADDING[0] = 0x80
local MD5_CTX_state = ffi.typeof("unsigned int[4]")
...
local function decode(output, input, len)
	...
	while j < len do
		output[i] = bit.bor(bit.lshift(input[j + 3], 24), bit.lshift(input[j + 2], 16), bit.lshift(input[j + 1], 8), input[j])
		i = i + 1
		j = j + 4
	end
end
...
local const_u_char = ffi.typeof("const unsigned char*")
...
function md5.string(str)
	...
	local b = bit.lshift(digest[4], 24) + bit.lshift(digest[5], 16) + bit.lshift(digest[6], 8) + digest[7]
	local c = bit.lshift(digest[8], 24) + bit.lshift(digest[9], 16) + bit.lshift(digest[10], 8) + digest[11]
	local d = bit.lshift(digest[12], 24) + bit.lshift(digest[13], 16) + bit.lshift(digest[14], 8) + digest[15]

	return bit.tohex(a) .. bit.tohex(b) .. bit.tohex(c) .. bit.tohex(d)
end
